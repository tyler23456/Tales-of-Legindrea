...
{
    public class SidePropInstantiator
    {
        ...
        public void Instantiate(HashSet<Vector2Int> pointSet, HashSet<Vector2Int> floorSet, List<GameObject> prefabs, List<Vector3> offsets)
        {
            ...
            foreach (Vector2Int p in pointSet)
            {
                if (!pointSet.Contains(p + up) && !pointSet.Contains(p + left)
                    || !pointSet.Contains(p + up) && !pointSet.Contains(p + right)
                    || !pointSet.Contains(p + down) && !pointSet.Contains(p + left)
                    || !pointSet.Contains(p + down) && !pointSet.Contains(p + right))
                {
                    continue;
                }

                if (!pointSet.Contains(p + up))
                {
